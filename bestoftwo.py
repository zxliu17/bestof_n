# -*- coding: utf-8 -*-
"""
Created on Sun Jul  8 14:44:25 2018
...
#'H2': 7, 'H3': 8, 'H4': 6,'H5': 3,'H6': 2,'H7': 4,'H8':5,'H9':1}
...
agent_number =50
N=10
figurename = 'Neq2_norm1_'+str(agent_number)+'_'+str(iter)+'_'+str(N)

path = 'figsTwo1/'

def iteration(agents,agent_number, iteration_times,Q):
    cardinality = []
    P_choosing_best =[] 
    #an = agent_number
...
#cc=[x[-1] for x in bestPstore]
    
figsize = 6,4
fig = plt.figure()
ax = Axes3D(fig)
ax.plot(Quality0,Quality1, aveP)#, rstride=0.1, cstride=0.1, cmap=plt.get_cmap('rainbow'))

fig2 = plt.figure()
for i in range(len(Quality0)):
    plt.scatter(Quality1[i],Quality0[i],s = 100*propP1[i],color = 'skyblue')
    plt.text(Quality1[i],Quality0[i], '%.2f' % propP1[i], ha='center', va= 'bottom',fontsize=7)
plt.savefig(path+'Average_p1'+figurename+'.pdf')
plt.show()

info = []
for i in range(len(Quality0)):
    info.append([Quality0[i],Quality1[i], propP1[i]])
#Quality0=np.array(Quality0)
